...
from preprocessing import BinningParamsMultiVars, MapDictMultiVars, preprocess_df
...
from utils.framework_depends import (
    ...
    rename_columns, convert_df_to_polars, concat_df, convert_df_to_pandas
)
...
from ._polars import (
    ...
    make_reverse_mapping_pl, filter_by_population_pl, filter_by_tgrate_pl
)
...
def calc_concentration_report(
    ...
    full_report, dict_segments = [], dict()
    for cnt in range(combo_min, combo_max + 1):
        for vars_combo in itertools.combinations(analyze_vars, cnt):
            start_id_segment = len(dict_segments) + 1
            combo_report, combo_segmets = _calc_concentration_combo(df, vars_combo, target_name,
                                                                    vars_order, start_id_segment)
            dict_segments = {**dict_segments, **combo_segmets}
    ...
    def get_nearest_segments(id_segment):
    ...
    if framework is FrameWork.polars:
        ...
        full_report = merge_nearest_segments_pl(full_report, get_nearest_segments)
...
def _calc_concentration_combo(df: DataFrame, analyze_vars: List[str], target_name: str,
                              vars_order: Dict[str, int], start_id: int):
    analyze_vars = sorted(analyze_vars)
    framework = get_framework_from_dataframe(df)
    func = _MAP_FRAMEWORK_calc_concentration[framework]

    concentr_stats, segmets_dict = func(df, analyze_vars, target_name, vars_order, start_id)
    rename_dict = {}
    names_cols = []
    values_cols = []
    ...
    return concentr_stats, segmets_dict
