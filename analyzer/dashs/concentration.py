...
class ConcentrationDash:
    """
    Дашборд для поиска концентрации и деконцентрации Target-a в данных.
    ...
    Args:
        ...
        min_bin:                    Минимальный размер бакета для бинаризации.
        bad_cols_start:             Колонки имеющие, число категорий превышающее заданное значение,
                                    не будут участвовать в анализе.
        max_cached_reports:         Максимальное количество закэшированных отчетов.

    Применение:
    1. Подали любой датафрейм на вход, бинаризавоать колонки не нужно, это будет происходить под капотом.
       По желанию задали дополнительные параметры
    2. Настроили виджеты - Комбо, Мин. бакет, Таргет. Опционально можно настроит и Мин. IV (далее расскажу на что это влияет)
        Желательно максимальное комбо устанавливать не выше 4-х, так как расчет может занять большое количество времени.
    3. Нажать на кнопку рассчитать сегменты и подождать пока выполнятся расчеты.
    4. После подготовки отсчета в виджете Сегмент вы можете увидеть большое множество сегментов, отсортированных
       по абсолютному значению IV статистики. Чтобы ограничить выбор, оставив только топ, вы можете установить
       минимальное значение для абсолютного значения IV в соответствующем виджете.
    5. Выберет интересующий вас сегмент. В ответ вы получите отчет по данному сегменту в виде графика и таблицы.
    """
    ...
    def _update_segment(self):
        ...
        sub_df = preprocess_df(
            sub_df, select_cols, ignore_vars=None, target_name=self._target.value,
            map_values=None, binning=BinningParams(min_prc=self._min_bucket.value),
            drop_not_processed=False, _validate_target=False,
